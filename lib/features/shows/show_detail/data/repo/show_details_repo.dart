...
class ShowDetailsRepo {
  ...
  ShowDetailsRepo(this._apiService);

  Future<Show> getShow({required int id}) async => Show.fromJson(
    (await _apiService.get(
      url: EndpointHelper.getEndPoint(id: id, typeIndex: 1),
      useBaseUrl: false,
    )).data,
  );
  ...
  Future<List<Cast>> getCast({required int id}) => _apiService.fetchList(
    url: EndpointHelper.getEndPoint(id: id, category: '/credits', typeIndex: 1),
